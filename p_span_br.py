...
def scrape_thai_law(url, output_file, append_mode=False):
    ...
    with open(output_file, mode, encoding='utf-8') as file:
        for p in article_div.find_all('p'):
            text_parts = []
            for content in p.descendants:
                if content.name == 'br':
                    text_parts.append('\n')  # Add a single newline for <br> tags
                elif isinstance(content, str):
                    content = content.replace(u'\xa0', ' ')  # Replace &nbsp; with space
...
if __name__ == "__main__":
    ...
    scrape_thai_law(url, output_file, append_mode=True)